...
public class EmployeeControllerClient {
	...
	public static void displayEmployeeById(String empId) {
		...
		EmployeeModel model = (EmployeeModel) client.jsonToObject(response, EmployeeModel.class);
	...
	public static void displayEmployeeList() {
		...
		List<EmployeeModel> modelList = (List<EmployeeModel>) client.jsonToObjectList(response);
	...
	private Object jsonToObjectList(ResponseEntity<?> response) {
		...
		List<Object> list = new Gson().fromJson(response.getBody().toString(), listType);
